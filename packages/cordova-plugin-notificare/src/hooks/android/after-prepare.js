...
const gradleConfigDefaults = require('./notificare-gradle-config-defaults');
...
function updateUserProjectGradleConfig(context) {
  ...
  const projectGradleConfigPath = path.join(platformRoot, 'notificare-gradle-config.json');
...
function getUserGradleConfig(configXml) {
  const configXmlToGradleMapping = [
    {
      xmlKey: 'NotificareServicesGradlePluginVersion',
      gradleKey: 'NOTIFICARE_SERVICES_GRADLE_PLUGIN_VERSION',
      type: String,
    },
  ];
